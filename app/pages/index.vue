<template>
  <div class="min-h-dvh grid grid-cols-[72px_1fr_320px] grid-rows-[auto_1fr]">
    ...
    <EditorToolbar
      ...
      @update:tool="(t: 'upload' | 'adjust' | 'filter' | 'resize' | 'export') => store.setTool(t)"
      ...
      <div class="w-full h-full flex items-center justify-center p-4">
        ...
        >
          ...
          <div
            ...
            :class="{
              'cursor-grab': imageLoaded && !isPanning,
...
function screenToImageCoordinates(screenX: number, screenY: number) {
  ...
  if (
    screenX < imgLeft ||
    screenX > imgLeft + imgScreenWidth ||
    screenY < imgTop ||
    screenY > imgTop + imgScreenHeight
  ) {
    return null;
  }

  // Convert to image coordinates (accounting for transformations)
  let imgX = (screenX - imgLeft) / scale.value;
  let imgY = (screenY - imgTop) / scale.value;
